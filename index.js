...
const { Client, GatewayIntentBits, ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, ActivityType, StringSelectMenuBuilder, ComponentType, MessageFlags, REST, Routes, SlashCommandBuilder } = require('discord.js');
  ...
  try {
    ...
    const commands = [
      new SlashCommandBuilder()
        .setName('ping')
        .setDescription('Shows all bot latency metrics in ms')
        .toJSON()
    ];
...
if (riffy) {
  riffy.on('nodeConnect', (node) => {
    const wasReconnect = lavalinkConnected === false && reconnectTimer !== null;
    ...
    // On reconnect, destroy stale players that have broken voice connections
    if (wasReconnect) {
      for (const [guildId, player] of riffy.players) {
        try {
          player.destroy();
...
client.on('messageCreate', async (message) => {
  ...
  if (command === 'play') {
    ...
    try {
      ...
      if (resolve.loadType === 'playlist') {
        ...
        const queuePosition = (!player.playing && !player.paused) ? '0' : `${player.queue.length}`;
        const embed = new EmbedBuilder()
          .setColor(config.color.info)
      ...
      const embed = new EmbedBuilder()
        .setColor(config.color.error)
        .setDescription('❌ An error occurred while loading the track. Please try again.');
      message.reply({ embeds: [embed] });
    }
  }
  ...
  // BOTINFO Command
  if (command === 'botinfo') {
    const botInfoUsers = client.guilds.cache.reduce((acc, guild) => acc + guild.memberCount, 0);
    const embed = new EmbedBuilder()
      .setColor(config.color.info)
      ...
      .addFields(
        ...
        { name: 'Users', value: `${botInfoUsers.toLocaleString()}`, inline: true },
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'ping') {
    ...
    // Gateway / Heartbeat / Bot / Shard latency — all sourced from WebSocket ping
    const wsLatency = client.ws.ping;
    const shardLatency = interaction.guild?.shard?.ping ?? wsLatency;

    // API latency — time for a live REST round-trip
    const apiStart = Date.now();
    await interaction.editReply({ content: '📡 Measuring...' });
    const apiLatency = Date.now() - apiStart;

    // Roundtrip latency — total time from command received to final reply sent
    const roundtripLatency = Date.now() - commandStart;

    // Command execution latency — same window, labeled separately
    const commandExecLatency = roundtripLatency;
    ...
    const fmt = (ms) => ms !== null && ms !== undefined ? `**${ms}ms**` : '`N/A`';

    const embed = new EmbedBuilder()
      .setColor(config.color.info)
      .setTitle('📡 /ping — Full Latency Report')
      .addFields(
        { name: `${emo(wsLatency)} Bot Latency`,                value: fmt(wsLatency),          inline: true },
        { name: `${emo(wsLatency)} Gateway Latency`,            value: fmt(wsLatency),          inline: true },
        { name: `${emo(wsLatency)} Heartbeat Latency`,          value: fmt(wsLatency),          inline: true },
        { name: `${emo(shardLatency)} Shard Latency`,           value: fmt(shardLatency),       inline: true },
        { name: `${emo(apiLatency)} API Latency`,               value: fmt(apiLatency),         inline: true },
        { name: `${emo(roundtripLatency)} Roundtrip Latency`,   value: fmt(roundtripLatency),   inline: true },
        { name: `${emo(commandExecLatency)} Command Execution`, value: fmt(commandExecLatency), inline: true },
        { name: '🔵 Lavalink Status', value: lavalinkConnected ? '✅ Online' : '❌ Offline',   inline: true }
      )
      .setFooter({ text: `🟢 <100ms  🟡 <200ms  🔴 200ms+` });
